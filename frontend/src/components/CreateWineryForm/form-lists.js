export const staticVarietalList = [
  { label: "Melon de Bourgogne", value: 1 },
  { label: "Chenin Blanc", value: 2 },
  { label: "Cabernet Franc", value: 3 },
  { label: "Romorantin", value: 4 },
  { label: "Chardonnay", value: 5 },
  { label: "Sauvignon Blanc", value: 6 },
  { label: "Folle Blanche", value: 7 },
  { label: "Tressailier", value: 8 },
  { label: "Pinot Noir", value: 9 },
  { label: "Gamay", value: 10 },
  { label: "Grolleau", value: 11 },
  { label: "Côt/Malbec", value: 12 },
  { label: "Pineau d'Aunis", value: 13 },
];

export const staticAmenityList = [
  { label: "Organic", value: 1 },
  { label: "Biodynamic", value: 2 },
  { label: "Natural Winery", value: 3 },
  { label: "Beautiful Tasting Room", value: 4 },
  { label: "Spectacular Views", value: 5 },
  { label: "English Speaking", value: 6 },
  { label: "Woman-led", value: 7 },
  { label: "Barrel Caves", value: 8 },
  { label: "Urban Location", value: 9 },
  { label: "Library Wines", value: 10 },
  { label: "Great for Groups", value: 11 },
  { label: "Legendary Producer", value: 12 },
];

export const staticWineStyleList = [
  { label: "Sparkling", value: 1 },
  { label: "White", value: 2 },
  { label: "Orange/Skin-Contact", value: 3 },
  { label: "Rosé", value: 4 },
  { label: "Red", value: 5 },
  { label: "Dessert/Fortified", value: 6 },
];

export const staticRegionList = [
  { label: "Pays Nantais", id: 1 },
  { label: "Anjou", id: 2 },
  { label: "Saumur", id: 3 },
  { label: "Touraine", id: 4 },
  { label: "Coteaux-du-Giennois", id: 5 },

  { label: "Sancerre", id: 6 },
  { label: "Pouilly-fumé/Pouilly-sur-Loire", id: 7 },
  { label: "Menetou Salon", id: 8 },
  { label: "Valençay", id: 9 },
  { label: "Quincy", id: 10 },

  { label: "Reuilly", id: 11 },
  { label: "Châteaumeillant", id: 12 },
  { label: "Saint-Pourçain", id: 13 },
  { label: "Côtes Roannaises", id: 14 },
  { label: "Côtes d’Auvergne", id: 15 },

  { label: "Côtes du Forez", id: 16 },
  { label: "Vouvray", id: 17 },
  { label: "Chinon", id: 18 },
  { label: "Bourgueil", id: 19 },
];

export const staticTimeList = [
  { label: "11:00" },
  { label: "13:00" },
  { label: "14:00" },
  { label: "15:00" },
  { label: "16:00" },
  { label: "17:00" },
];
